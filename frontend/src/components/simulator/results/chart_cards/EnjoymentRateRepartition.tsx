    ...
    return (
        <div className="card bg-white! p-8 gap-6 flex flex-col items-center">
            ...
            <div className="flex flex-col gap-3 w-full">
                <CollapsibleBadge color={missions.length > 0 ? ENJOYMENT_COLOR : DISABLED_COLOR} title={`${missions_average.toLocaleString('fr-FR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} / ${5}`} description={t("simulator.inputs.tabs.mission.name")}>
                    {
                        missions.length > 0 && <div className="flex gap-3 flex-col">
                            {missions.map(({ name, enjoyment_rate }) => <span className="flex gap-3"><p className="font-bold min-w-9">{`${enjoyment_rate.toLocaleString('fr-FR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} / ${5}`}</p><p>{name}</p></span>)}
                        </div>
                    }
                </CollapsibleBadge>
                <CollapsibleBadge color={projects.length > 0 ? ENJOYMENT_COLOR : DISABLED_COLOR} title={`${projects_average.toLocaleString('fr-FR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} / ${5}`} description={t("simulator.inputs.tabs.project.name")}>
                    {
                        projects.length > 0 && <div className="flex gap-3 flex-col">
                            {projects.map(({ name, enjoyment_rate }) => <span className="flex gap-3"><p className="font-bold min-w-9">{`${enjoyment_rate.toLocaleString('fr-FR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} / ${5}`}</p><p>{name}</p></span>)}
                        </div>
                    }
                </CollapsibleBadge>
            </div>
